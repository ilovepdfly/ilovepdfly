        ...
        <div className="bg-white dark:bg-black p-8 rounded-lg shadow-lg w-full">
            ...
            {files.map(file => (
                <div key={file.name} className="flex items-center justify-between p-3 mb-2 bg-gray-100 dark:bg-black dark:border dark:border-gray-800 rounded-md">
                    ...
                    <button onClick={() => removeFile(file.name)} className="text-gray-400 hover:text-red-500 transition-colors flex-shrink-0 ml-2" aria-label={`Remove ${file.name}`}>
            ...
            <div className="flex justify-center mt-6">
                <div {...addMoreFilesProps.getRootProps()} className="inline-block">
                    ...
                    <button type="button" className={`flex items-center gap-2 text-white font-bold py-2 px-6 rounded-lg text-md transition-colors ${tool.color} ${tool.hoverColor}`}>
