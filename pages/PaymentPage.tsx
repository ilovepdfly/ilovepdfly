...
const PaymentPage: React.FC = () => {
    ...
    const { user } = useAuth();
    const plan = (location.state?.plan || 'premium') as 'premium' | 'pro';
    const planDetails = {
    ...
    const handleConfirm = async () => {
        ...
        const message = `Hello! I've made a payment for the ${planDetails[plan].name} plan on ILovePDFLY.\nMy username is: ${user?.username || 'Not logged in'}.\nPlease activate my account.\n\nWebsite: https://ilovepdfly.com/`;
                ...
                <div className="max-w-3xl mx-auto bg-white dark:bg-black p-6 md:p-8 rounded-lg shadow-xl space-y-8">
                    ...
                    <div>
                        ...
                        <div className={`pl-12 ${currentStep !== 1 ? 'hidden' : ''}`}>
                            <p className="mb-4 text-gray-600 dark:text-gray-300">Scan the Fonepay QR code below to pay <strong>{planDetails[plan].price}</strong> for the <strong>{planDetails[plan].name}</strong> plan. Don't forget to take a screenshot of the successful payment confirmation.</p>
                            <div className="text-center p-4 border border-gray-200 dark:border-gray-700 rounded-lg max-w-xs mx-auto">
                                <img src="https://ik.imagekit.io/fonepay/fonepay%20qr.png?updatedAt=1752920160699" alt="Fonepay QR Code" className="w-48 h-48 mx-auto" />
