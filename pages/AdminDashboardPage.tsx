...
import { UserIcon, StarIcon, TrashIcon } from '../components/icons.tsx';
...
const AdminDashboardPage: React.FC = () => {
    const { getAllUsers, updateUserPremiumStatus, logout, deleteUser } = useAuth();
                            ...
                            <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                                ...
                                <tbody>
                                    {loading ? (
                                        ...
                                        filteredUsers.map(user => (
                                            <tr key={user.username} className="bg-white dark:bg-gray-900 border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800/50">
                                                ...
                                                <td className="px-6 py-4">
                                                    <span className={`px-2 py-1 text-xs font-semibold rounded-full ${user.isPremium ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300' : 'bg-gray-200 text-gray-800 dark:bg-gray-700 dark:text-gray-300'}`}>
                                       ...
                                       <tr><td colSpan={4} className="text-center p-6">No users found.</td></tr>
                                    )}
                                </tbody>
                            </table>
                        </div>
